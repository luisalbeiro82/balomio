...
1. ~~**Estadísticas de números que más salen**~~ ✅ HECHO (2026-07-09):
   ...
   - **Semilla histórica añadida (2026-07-09):** `sorteos-baloto.json` en la raíz (216 sorteos oficiales de Baloto ene-2025 → jul-2026), digitados de resultadobaloto.com/resultados.php (2026) y resultados-de-loteria.com/baloto/resultados/2025. Se carga con `fetch` (mapa `SEMILLAS`, solo Baloto por ahora) y se **combina** con los sorteos del usuario vía `sorteosParaEstadisticas()` (dedup por combinación). La nota lo declara como "historial oficial 2025–2026". Para actualizar/ampliar: regenerar el JSON con más años (mismo esquema `{fecha, numeros[5], superbalota}`). MiLoto no tiene semilla (no había fuente); crece solo.
